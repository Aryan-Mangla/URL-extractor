...
function download_table_as_csv($urls) {
    ...
    $header = [
        ...
        'Arrivals', 'Departures', 'ICAO Code','IATA Code', 'Ticket Counter Hours', 'Official Website', 'Official Facebook',
        'Official Twitter', 'Official YouTube', 'Official Instagram', 'Iframe'
    ];
    fputcsv($file, $header);
    ...
    foreach ($urls as $url) {
        ...
        if (!empty($url)) {
            ...
            // Find all unordered lists with list items
            $uls = $html->find('ul');
            foreach ($uls as $ul) {
                foreach ($ul->find('li') as $li) {
                    // Split key and value by colon
                    $parts = explode(':', $li->plaintext, 2);
                    if (count($parts) == 2) {
                        $key = trim($parts[0]);
                        $value = trim($parts[1]);
                        $tableData[$key] = $value;
            ...
            $fields = [
                'Airport Name' => ['Airport Name','Name of the Airport','Airport'],
